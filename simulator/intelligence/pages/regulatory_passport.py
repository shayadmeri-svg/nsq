...
def _render_passport(passport: dict) -> None:
    ...
    st.markdown(
        f"""
        <div style="border:1px solid #e2e8f0; border-radius:6px; padding:16px; background:#ffffff; margin-bottom:16px;">
          <div style="display:flex; justify-content:space-between; align-items:flex-start;">
            <div>
              <div class="cdmo-section-title">Reference Listed Drug</div>
              <div style="font-size:18px; font-weight:800; color:#0f172a; margin-top:2px;">{rld or '—'}</div>
              <div style="font-size:11px; color:#64748b; margin-top:2px;">{applicant or '—'}</div>
            </div>
          ...
          <div style="display:grid; grid-template-columns:repeat(3, 1fr); gap:12px; margin-top:16px;">
            <div><div style="font-size:10px; color:#94a3b8;">Dosage Form</div>
            <div style="font-size:13px; font-weight:600; color:#0f172a;">{dosage or '—'}</div></div>
            <div><div style="font-size:10px; color:#94a3b8;">Strength</div>
            <div style="font-size:13px; font-weight:600; color:#0f172a;">{strength or '—'}</div></div>
            <div><div style="font-size:10px; color:#94a3b8;">BCS Class</div>
            <div style="font-size:13px; font-weight:600; color:#0f172a;">{bcs or '—'}</div></div>
          </div>
        </div>
        """,
    ...
    for idx, (title, key) in enumerate(
        ...
        with cols[idx]:
            ...
            if text:
                st.markdown(
                    f"""
                    <div style="border:1px solid #e2e8f0; border-radius:4px; padding:10px; background:#f8fafc; height:100%;">
                      <div style="font-size:10px; font-weight:800; text-transform:uppercase; color:#64748b; margin-bottom:6px;">{title}</div>
                      <div style="font-size:11px; color:#475569; line-height:1.45;">{text}</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
    ...
    if specs:
        ...
        st.markdown(
            "".join(
                f"<span style='padding:2px 6px; border-radius:3px; background:#e2e8f0; color:#475569; font-size:10px; margin-right:6px;'>{s}</span>"
                for s in specs
            ),
            unsafe_allow_html=True,
        )
    ...
    if stability:
        ...
        st.markdown(f"<div style='font-size:12px; color:#475569;'>{stability}</div>", unsafe_allow_html=True)
    ...
    if exclusivity:
        ...
        for ex in exclusivity:
            ...
            st.markdown(
                f"""
                <div style="border-left:3px solid #ef4444; padding:8px 12px; background:#fef2f2; margin-bottom:6px;">
                  <div style="font-size:12px; font-weight:700; color:#991b1b;">{ex.get('type', '')} until {expiry}</div>
                  <div style="font-size:11px; color:#b91c1c; margin-top:2px;">{desc}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    if be_notes:
        ...
        st.markdown(f"<div style='font-size:12px; color:#475569;'>{be_notes}</div>", unsafe_allow_html=True)

    notes = passport.get("notes", "")
    if notes:
        st.markdown("#### Notes")
        st.markdown(f"<div style='font-size:12px; color:#475569;'>{notes}</div>", unsafe_allow_html=True)
