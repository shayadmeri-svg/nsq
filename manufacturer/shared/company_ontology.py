...
def canonical_state_name(name: str) -> str:
    """Canonical display name for an already-extracted state string.

    Unknown values pass through unchanged — this is a display-side
    defense so stale persisted values ('Uttaranchal', 'Orissa', 'Jammu
    And Kashmir') render on the modern 36-state map instead of
    vanishing, even before the Redis they came from is rebuilt.
    """
    if not name:
        return ""
    key = _strip_accents(str(name)).lower()
    return _STATE_CANONICAL.get(key, str(name))
