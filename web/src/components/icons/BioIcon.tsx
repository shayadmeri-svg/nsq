// Monochrome scientific bioicons — port of manufacturer/ui/icons.py.
// Each icon uses stroke="currentColor" so it inherits text color and stays
// monochrome, per the UI/UX tone rule (no emoji, no colored Figma icons).
// lucide-react is used only for generic chrome (chevrons, arrows), never as
// a section icon.
...
export function MoleculeIcon(p: IconProps) {
  return (
    <svg {...svg(p)}>
...
export function QmarkIcon(p: IconProps) {
  return (
    <svg {...svg({ strokeWidth: 1.6, ...p })}>
...
// Registry mapping section/icon names -> components, mirroring the Python
// bioicon(name) lookup with a molecule fallback.
...
export type BioiconName = keyof typeof BIOICONS;

export function BioIcon({ name, size = 20, ...rest }: { name: BioiconName; size?: number } & SVGProps<SVGSVGElement>) {
  const Cmp = BIOICONS[name] ?? MoleculeIcon;
