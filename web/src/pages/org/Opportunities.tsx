...
import { ArrowRight, CalendarClock, Lightbulb, Rocket, Sparkles } from "lucide-react";
        ...
        <div className="space-y-5">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl border border-line p-4">
              <div className="label mb-2">Loss of exclusivity</div>
              <Loe loe={{ in: data.patent.estimated_loe_in, eu: data.patent.estimated_loe_eu, us: data.patent.estimated_loe_us }} />
              <div className="mt-3 flex flex-wrap gap-1.5"><Badge tone={data.patent.fto_risk === "high" ? "rose" : data.patent.fto_risk === "medium" ? "amber" : "brand"}>FTO risk: {data.patent.fto_risk}</Badge>{data.patent.market_size_usd_bn && <Badge tone="indigo">${data.patent.market_size_usd_bn} bn market</Badge>}</div>
              <p className="mt-3 line-clamp-4 text-xs leading-relaxed text-ink-muted">{data.patent.notes}</p>
            </div>
            {fit ? (
              <div className="rounded-2xl border border-line p-4">
                <div className="flex items-center justify-between"><div className="label">Fit with {data.plant.name}</div><span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ring-1 ring-inset ${TIER_STYLE[fit.commercial_fit_tier]}`}>{titleCase(fit.commercial_fit_tier)}</span></div>
...
export function Opportunities() {
  ...
  const rows = useMemo(() => (data?.molecules ?? []).filter((m: any) => tier === "all" || m.fit_tier === tier), [data, tier]);
  ...
  return (
    <>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Stat label="Available within 5 years" value={data.ready_within_5y} icon={<CalendarClock size={18} />} tone="indigo" delay={0.05} hint="good fit and off-patent or LOE ≤ 5 y" />
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1.5fr_1fr]">
        <Card delay={0.1}>
          <CardHeader title="Patent cliff × plant fit" subtitle="Bubble size = market size. Click a molecule for its roadmap." />
            ...
            {data.unlocks.slice(0, 6).map((u: any) => (
              <motion.li variants={itemVariants} key={u.kind + u.token} className="rounded-xl border border-line p-3.5">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="text-right"><div className="font-display text-lg font-extrabold text-brand-700">+{u.count}</div><div className="text-[11px] text-ink-muted">{u.market_usd_bn ? `$${u.market_usd_bn} bn` : "molecules"}</div></div>
                </div>
      ...
      <Card delay={0.2} className="mt-5">
        <CardHeader title="All molecules" subtitle={`${rows.length} shown`} action={
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-y border-line bg-slate-50/70 text-left text-[11px] font-semibold uppercase tracking-wider text-ink-muted">
              <th className="px-5 py-2.5">Molecule</th><th className="px-3 py-2.5">Loss of exclusivity</th><th className="px-3 py-2.5">Best plant</th><th className="px-3 py-2.5">Fit</th><th className="px-3 py-2.5">Missing</th><th className="px-5 py-2.5 text-right">Score</th>
            </tr></thead>
            <tbody>
              {rows.map((m: any) => (
                <tr key={m.molecule_key} onClick={() => nav(`/o/${slug}/opportunities/${m.molecule_key}`)} className="cursor-pointer border-b border-line/70 transition hover:bg-brand-50/40">
                  <td className="px-5 py-3"><div className="font-semibold">{m.api_name}</div><div className="text-xs text-ink-muted">{m.therapeutic_area} · {titleCase(m.modality)}{m.market_size_usd_bn ? ` · $${m.market_size_usd_bn} bn` : ""}</div></td>
